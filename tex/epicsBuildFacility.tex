...
\begin{itemize}\item \index{dbd directory}dbd - Directory into which Database Definition files are installed.

\item \index{include directory}include - The directory into which C header files are installed. These header files may be generated from menu and 
record type definitions.

\item \index{bin directory}bin - This directory contains a subdirectory for each host architecture and for each target architecture. These are the 
directories into which executables, binaries, etc. are installed.

\item \index{lib directory}lib - This directory contains a subdirectory for each host architecture. These are the directories into which libraries 
are installed.

\item \index{db directory}db - This is the directory into which database record instance, template, and substitution files are installed.

\item \index{html directory}html - This is the directory into which html documentation is installed.

\item \index{templates directory}templates - This is the directory into which template files are installed.

\item \index{javalib directory}javalib - This is the directory into which java class files and jar files are installed.

\item \index{configure directory}configure - The directory into which configure files are installed (if INSTALL\_LOCATION not equal TOP).

\item \index{cfg directory}cfg - The directory into which user created configure files are installed 
...
\index{INSTALL\_LOCATION}INSTALL\_LOCATION\_LIB      = \$(INSTALL\_LOCATION)/lib
...
\index{RELEASE\_INCLUDES}RELEASE\_INCLUDES += -I/home/epics/modules/bus/camac/include/os

\index{RELEASE\_INCLUDES}RELEASE\_INCLUDES += -I/home/epics/modules/bus/camac/include

\index{RELEASE\_DBDFLAGS}RELEASE\_DBDFLAGS += -I /home/epics/modules/bus/camac/dbd
...
\begin{description}\item PROD = aaa
...
\textless{}home\textgreater{}/configure/\index{CONFIG\_USER}CONFIG\_USER
...
\end{verbatim}\index{gnumake}\begin{description}\item This rebuilds and installs everything that is not up to date.
...
\end{verbatim}\index{clean}\begin{description}\item This can be used to save disk space by deleting the O.\textless{}arch\textgreater{} directories that gnumake will create, but does not 
...
\end{verbatim}\index{archclean}\begin{description}\item This command will remove the current build's O.\textless{}arch\textgreater{} directories but not O.Common directory.

\item 

\end{description}\begin{verbatim}gnumake realclean
\end{verbatim}\index{realclean}\begin{description}\item This command will remove ALL the O.\textless{}arch\textgreater{} subdirectories (even those created by a gnumake from another 
EPICS\_HOST\_ARCH).

\item 

\end{description}\begin{verbatim}gnumake rebuild 
\end{verbatim}\index{rebuild}\begin{description}\item This is the same as "gnumake clean install". If you are unsure about the state of the generated files in an 
application, just execute "gnumake rebuild".

\item 

\end{description}\begin{verbatim}gnumake uninstall
\end{verbatim}\index{uninstall}\begin{description}\item This command can be executed from the \textless{}top\textgreater{} directory only. It will remove everything installed by gnumake in 
the include, lib, bin, db, dbd, etc. directories.

\item 

\end{description}\begin{verbatim}gnumake realuninstall
\end{verbatim}\index{realuninstall}\begin{description}\item This command can be executed from the \textless{}top\textgreater{} directory only. It will remove all the install directories, include, lib, 
bin, db, dbd, etc.

\item 

\end{description}\begin{verbatim}gnumake distclean
\end{verbatim}\index{distclean}\begin{description}\item This command can be executed from the \textless{}top\textgreater{} directory only. It is the same as issuing both the realclean and 
realuninstall commands.

\item 

\end{description}\begin{verbatim}gnumake cvsclean
\end{verbatim}\index{cvsclean}\begin{description}\item This command can be executed from the \textless{}top\textgreater{} directory only. It removes cvs .\#* files in the make directory tree.
...
\begin{description}\item \index{USR\_INCLUDES}USR\_INCLUDES += -I\textless{}name\textgreater{}
...
\begin{description}\item \index{USR\_CFLAGS}USR\_CFLAGS += \textless{}c flags\textgreater{}
...
\begin{description}\item USR\_CFLAGS\_DEFAULT += \textless{}c flags\textgreater{}

\end{description}c compiler options for any arch that does not have a USR\_CFLAGS\_\textless{}osclass\textgreater{} definition

\begin{description}\item \index{USR\_CXXFLAG}USR\_CXXFLAGS += \textless{}c++ flags\textgreater{}
...
\begin{description}\item \index{USR\_CPPFLAGS}USR\_CPPFLAGS += \textless{}preprocessor flags\textgreater{}
...
\begin{description}\item USR\_CPPFLAGS\_\textless{}T\_A\textgreater{} += \textless{}preprocessor flags\textgreater{}
...
\begin{description}\item \index{USR\_LDFLAGS}USR\_LDFLAGS += \textless{}linker flags\textgreater{}
...
\begin{description}\item \textless{}\index{name\_INCLUDES}name\textgreater{}\_INCLUDES += -I\textless{}name\textgreater{}
...
\begin{description}\item \textless{}name\textgreater{}\_INCLUDES\_\textless{}T\_A\textgreater{} += -I\textless{}name\textgreater{}

\end{description}target architecture specific header file directories each prefixed by a "-I".

\begin{description}\item \textless{}\index{name\_CFLAGS}name\textgreater{}\_CFLAGS += \textless{}c flags\textgreater{}
...
\begin{description}\item \textless{}\index{name\_CXXFLAGS}name\textgreater{}\_CXXFLAGS += \textless{}c++ flags\textgreater{}
...
\begin{description}\item \textless{}\index{name\_CPPFLAGS}name\textgreater{}\_CPPFLAGS += \textless{}preprocessor flags\textgreater{}
...
\begin{description}\item \textless{}\index{name\_LDFLAGS}name\textgreater{}\_LDFLAGS += \textless{}linker flags\textgreater{}
...
\begin{description}\item \index{LIBRARY\_IOC}LIBRARY += \textless{}name\textgreater{}
...
\begin{description}\item LIBRARY\_DEFAULT += \textless{}name\textgreater{}
...
\begin{description}\item \index{LIBRARY\_IOC}LIBRARY\_IOC += \textless{}name\textgreater{}
...
\begin{description}\item \index{LIBRARY\_HOST}LIBRARY\_HOST += \textless{}name\textgreater{}
...
\begin{description}\item \index{LIBSRCS}LIBSRCS += \textless{}name\textgreater{}
...
\begin{description}\item \index{USR\_SRCS}USR\_SRCS += \textless{}name\textgreater{}
...
\begin{description}\item \index{LIB\_SRCS}LIB\_SRCS += \textless{}name\textgreater{}
...
\begin{description}\item \textless{}\index{name\_SRCS}libname\textgreater{}\_SRCS += \textless{}name\textgreater{}

\end{description}Source files will be used for the named library.

\begin{description}\item \textless{}libname\textgreater{}\_SRCS\_\textless{}osclass\textgreater{} += \textless{}name\textgreater{}
...
\begin{description}\item \index{USR\_OBJS}USR\_OBJS += \textless{}name\textgreater{}
...
\begin{description}\item \index{LIB\_OBJS}LIB\_OBJS += \textless{}name\textgreater{}
...
\begin{description}\item \textless{}\index{name\_OBJS}libname\textgreater{}\_OBJS += \textless{}name\textgreater{}
...
\begin{description}\item \index{USR\_OBJLIBS}USR\_OBJLIBS += \textless{}name\textgreater{}
...
\begin{description}\item \index{LIB\_OBJLIBS}LIB\_OBJLIBS += \textless{}name\textgreater{}
...
\begin{description}\item \textless{}\index{name\_OBJLIBS}libname\textgreater{}\_OBJLIBS += \textless{}name\textgreater{}

\end{description}Combined object files will be used for all builds of the named library.

\begin{description}\item \verb|<libname>_OBJLIBS_<osclass> += <name>| 
...
\begin{description}\item \textless{}\index{name\_LDOBJS}libname\textgreater{}\_LDOBJS += \textless{}name\textgreater{}
...
\begin{description}\item LIBOBJS += \$(\textless{}support\textgreater{}\_BIN)/xxx.o
...
\begin{description}\item \index{LIB\_LIBS}LIB\_LIBS += \textless{}name\textgreater{}
...
\begin{description}\item LIB\_LIBS\_DEFAULT += \textless{}name\textgreater{}
...
\begin{description}\item \index{USR\_LIBS}USR\_LIBS += \textless{}name\textgreater{}
...
\begin{description}\item \textless{}\index{name\_LIBS}libname\textgreater{}\_LIBS += \textless{}name\textgreater{}
...
\begin{description}\item \verb|<libname>|\_LIBS\_DEFAULT += \textless{}name\textgreater{}
...
\begin{description}\item \textless{}\index{name\_SYS\_LIBS}libname\textgreater{}\_SYS\_LIBS += \textless{}name\textgreater{}
...
\begin{description}\item \index{DLL\_LIBS}DLL\_LIBS += \textless{}name\textgreater{}

\end{description}These DLLs will be used for all libraries.

\begin{description}\item \textless{}\index{name\_DLL\_LIBS}libname\textgreater{}\_DLL\_LIBS += \textless{}name\textgreater{}
...
\begin{description}\item \index{SHRLIB\_VERSION}SHRLIB\_VERSION += \textless{}version\textgreater{}
...
\index{Library example}\begin{description}\item LIBRARY\_vxWorks += vxWorksOnly
...
\index{Loadable libraries}Loadable libraries are regular libraries which are not required to have all symbols resolved during the build. The intent is 
...
\begin{description}\item \index{LOADABLE\_LIBRARY\_HOST}LOADABLE\_LIBRARY\_HOST += \textless{}name\textgreater{}
...
\begin{description}\item LOADABLE\_LIBRARY\_HOST\_DEFAULT += \textless{}name\textgreater{}
...
\begin{description}\item \index{SCRIPT}SCRIPTS += \textless{}name\textgreater{}
...
\begin{description}\item \index{SCRIPT\_IOC}SCRIPTS\_IOC += \textless{}name\textgreater{}

\end{description}Script \textless{}name\textgreater{} will be installed for IOC type archs.

\begin{description}\item SCRIPTS\_IOC\_\textless{}osclass\textgreater{} += \textless{}name\textgreater{}
...
\begin{description}\item \index{SCRIPT\_HOST}SCRIPTS\_HOST += \textless{}name\textgreater{}
...
\begin{description}\item \index{PROD}PROD += \textless{}name\textgreater{}
...
\begin{description}\item \index{PROD\_IOC}PROD\_IOC += \textless{}name\textgreater{}
...
\begin{description}\item \index{PROD\_HOST}PROD\_HOST += \textless{}name\textgreater{}
...
\begin{description}\item \index{PROD\_OBJS}PROD\_OBJS += \textless{}name\textgreater{}
...
\begin{description}\item \textless{}\index{name\_OBJS}prodname\textgreater{}\_OBJS += \textless{}name\textgreater{}
...
\begin{description}\item \index{PROD\_OBJLIBS}PROD\_OBJLIBS += \textless{}name\textgreater{}
...
\begin{description}\item \textless{}\index{name\_OBJLIBS}prodname\textgreater{}\_OBJLIBS += \textless{}name\textgreater{}

\end{description}Combined object files will be used for all builds of the named product.

\begin{description}\item \verb|<prodname>_OBJLIBS_<osclass> += <name>| 
...
\begin{description}\item \textless{}\index{name\_LDOBJS}prodname\textgreater{}\_LDOBJS += \textless{}name\textgreater{}
...
\begin{description}\item \index{PROD\_SRCS}PROD\_SRCS += \textless{}name\textgreater{}

\end{description}Source files will be used for all products.

\begin{description}\item PROD\_SRCS\_\textless{}osclass\textgreater{} += \textless{}name\textgreater{}
...
\begin{description}\item \textless{}\index{name\_SRCS}prodname\textgreater{}\_SRCS += \textless{}name\textgreater{}
...
\begin{description}\item \index{PROD\_LIBS}\index{PROD\_LIBS}PROD\_LIBS += \textless{}name\textgreater{}
...
\begin{description}\item \textless{}\index{name\_LIBS}prodname\textgreater{}\_LIBS += \textless{}name\textgreater{}
...
\begin{description}\item \index{SYS\_PROD\_LIBS}\index{SYS\_PROD\_LIBS}SYS\_PROD\_LIBS += \textless{}name\textgreater{}
...
\begin{description}\item \textless{}\index{name\_SYS\_LIBS}prodname\textgreater{}\_SYS\_LIBS += \textless{}name\textgreater{}
...
\begin{description}\item \index{TESTPROD}TESTPROD += \textless{}name\textgreater{}
...
\begin{description}\item \index{TESTPROD\_HOST}TESTPROD\_HOST += \textless{}name\textgreater{}
...
\begin{description}\item \index{TESTSCRIPTS}TESTSCRIPTS += \textless{}name\textgreater{}
...
\begin{description}\item TESTSCRIPTS\_IOC\_DEFAULT += \textless{}name\textgreater{}
...
\begin{description}\item \index{TESTSCRIPTS\_HOST}TESTSCRIPTS\_HOST += \textless{}name\textgreater{}
...
\begin{description}\item \index{JAVA}JAVA += \textless{}name\textgreater{}.java
...
\begin{description}\item \index{TESTJAVA}TESTJAVA += \textless{}name\textgreater{}.java
...
\begin{description}\item \index{USR\_JAVACFLAGS}USR\_JAVACFLAGS += \textless{}name\textgreater{}
...
\begin{description}\item \index{JAVA}JAVA = mytest/one.java
...
\begin{description}\item \index{TESTJAVA}TESTJAVA = test.java

\end{description}\subsection{Java jar file}

\index{Java jar file}A single java jar file can be created using the java jar tool and installed into \$(INSTALL\_JAVA) (i.e. 
...
\begin{description}\item \index{JAR}JAR += \textless{}name\textgreater{}

\end{description}The \textless{}name\textgreater{} jar file will be created and installed into the \$(INSTALL\_JAVA) directory.

\begin{description}\item \index{JAR\_INPUT}JAR\_INPUT += \textless{}name\textgreater{}

\end{description}Names of images, audio files and classes files to be included in the jar file.

\begin{description}\item \index{JAR\_MANIFEST}JAR\_MANIFEST += \textless{}name\textgreater{}
...
\begin{description}\item \index{JAR}JAR = mytest1.jar
...
\begin{description}\item \index{JAVAINC}JAVAINC += \textless{}name\textgreater{}.h

\end{description}    The \textless{}name\textgreater{}.h header file will be created in the O.Common subdirectory.

\begin{description}\item \index{USR\_JAVAHFLAGS}USR\_JAVAHFLAGS += \textless{}name\textgreater{}
...
\index{Table of Makefile definitions}Definitions given below containing \textless{}osclass\textgreater{} are used when building for target archs of a specific \index{osclass}osclass, and the 
...
\end{verbatim}\index{CONFIG.CrossCommon}\begin{description}\item Definitions for all hosts and all targets for a cross build (host different than target).

\end{description}\begin{verbatim}CONFIG.gnuCommon
\end{verbatim}\index{CONFIG.gnuCommon}\begin{description}\item Definitions for all hosts and all targets for builds using the gnu compiler.

\end{description}\begin{verbatim}CONFIG_ADDONS
\end{verbatim}\index{CONFIG\_ADDONS}\begin{description}\item Definitions which setup the variables that have \textless{}osclass\textgreater{} and DEFAULT options.
...
\end{verbatim}\index{CONFIG\_BASE}\begin{description}\item EPICS base specific definitions.

\end{description}\begin{verbatim}CONFIG_BASE_VERSION
\end{verbatim}\index{CONFIG\_BASE\_VERSION}\begin{description}\item Definitions for the version number of EPICS base. This file is used for creating epicsVersion.h which is installed 
into base/include.

\end{description}\begin{verbatim}CONFIG_COMMON
\end{verbatim}\index{CONFIG\_COMMON}\begin{description}\item Definitions common to all builds.

\end{description}\begin{verbatim}CONFIG_ENV
\end{verbatim}\index{CONFIG\_ENV}\begin{description}\item Default definitions of the EPICS environment variables. This file is used for creating envData.c which is included 
in the Com library.

\end{description}\begin{verbatim}CONFIG_FILE_TYPE
\end{verbatim}\index{CONFIG\_FILE\_TYPE}\begin{description}\item Definitions to allow user created file types.

\end{description}\begin{verbatim}CONFIG_SITE
\end{verbatim}\index{CONFIG\_SITE}\begin{description}\item File in which you add to or modify make variables in EPICS base. A definition normally overridden is:

\end{description}\begin{verbatim}CROSS_COMPILER_TARGET_ARCHS =
CONFIG_SITE_ENV
\end{verbatim}\index{CONFIG\_SITE\_ENV}\begin{description}\item Defaults for site specific definitions of EPICS environment variables. This file is used for creating envData.c 
which is included in the Com library.

\end{description}\begin{verbatim}CONFIG
\end{verbatim}\index{CONFIG}\begin{description}\item Include statements for all the other configure files. You can \index{override}override any definitions in other CONFIG* files by 
placing override definitions at the end of this file.

\end{description}\begin{verbatim}RELEASE
\end{verbatim}\index{RELEASE}\begin{description}\item Specifies the location of external products such as Tornado II and external \textless{}tops\textgreater{} such as EPICS base. 

\end{description}\begin{verbatim}RULES
\end{verbatim}\index{RULES}\begin{description}\item This file just includes the appropriate rules configuration file.

\end{description}\begin{verbatim}RULES.Db
\end{verbatim}\index{RULES.Db}\begin{description}\item Rules for building and installing database and database definition files. Databases generated from templates and/or 
CapFast schematics are supported.

\end{description}\begin{verbatim}RULES.ioc
\end{verbatim}\index{RULES.ioc}\begin{description}\item Rules which allow building in the iocBoot/\textless{}iocname\textgreater{} directory of a makeBaseApp created ioc application.

\end{description}\begin{verbatim}RULES_ARCHS
\end{verbatim}\index{RULES\_ARCHS}\begin{description}\item Definitions and rules which allow building the make target for each target architecture.

\end{description}\begin{verbatim}RULES_BUILD
\end{verbatim}\index{RULES\_BUILD}\begin{description}\item Build rules for the Makefiles 

\end{description}\begin{verbatim}RULES_DIRS
\end{verbatim}\index{RULES\_DIRS}\begin{description}\item Definitions and rules which allow building the make targets in each subdirectory. This file is included by Makefiles 
in directories with subdirectories to be built.

\end{description}\begin{verbatim}RULES_EXPAND
\end{verbatim}\index{RULES\_EXPAND}\begin{description}\item Definitions and rules to use \index{expandVars.pl}expandVars.pl to expand @VAR@ variables in a file.

\end{description}\begin{verbatim}RULES_FILE_TYPE
\end{verbatim}\index{RULES\_FILE\_TYPE}\begin{description}\item Definitions and rules to allow user created CONFIG* and RULES* files and rules to allow user created file types.

\end{description}\begin{verbatim}RULES_JAVA
\end{verbatim}\index{RULES\_JAVA}
\index{RULES\_JAVA}\begin{description}\item Definitions and rules which allow building java class files and java jar files.

\end{description}\begin{verbatim}RULES_TARGET
\end{verbatim}\index{RULES\_TARGET}\begin{description}\item Makefile code to create target specific dependency lines for libraries and product targets.

\end{description}\begin{verbatim}RULES_TOP
\end{verbatim}\index{RULES\_TOP}\begin{description}\item Rules specific to a \textless{}top\textgreater{} level directory e.g. uninstall and tar. It also includes the RULES\_DIRS file.

\end{description}\begin{verbatim}Makefile
\end{verbatim}\index{Makefile}
\index{Makefile}\begin{description}\item Definitions to allow creation of CONFIG\_APP\_INCLUDE and installation of the CONFIG* files into the 
...
\begin{verbatim}CONFIG.<host>.<target>
\end{verbatim}\index{CONFIG.\textless{}host\textgreater{}.\textless{}target\textgreater{}}\begin{description}\item Specific host-target build definitions

\end{description}\begin{verbatim}CONFIG.Common.<target>
\end{verbatim}\index{CONFIG.Common.\textless{}target\textgreater{}}\begin{description}\item Specific target definitions for all hosts

\end{description}\begin{verbatim}CONFIG.<host>.Common
\end{verbatim}\index{CONFIG.\textless{}host\textgreater{}.Common}\begin{description}\item Specific host definitions for all targets

\end{description}\begin{verbatim}CONFIG.UnixCommon.Common
\end{verbatim}\index{CONFIG.UnixCommon.Common}\begin{description}\item Definitions for Unix hosts and all targets

\end{description}\begin{verbatim}CONFIG.<host>.vxWorksCommon
\end{verbatim}\index{CONFIG.\textless{}host\textgreater{}.vxWorksCommon}\begin{description}\item Specific host definitions for all vx targets

\end{description}\begin{verbatim}CONFIG_COMPAT
\end{verbatim}\index{CONFIG\_COMPAT}\begin{description}\item R3.13 arch compatibility definitions

\end{description}\begin{verbatim}CONFIG_SITE.<host>.<target>
\end{verbatim}\index{CONFIG\_SITE.\textless{}host\textgreater{}.\textless{}target\textgreater{}}\begin{description}\item Site specific host-target definitions

\end{description}\begin{verbatim}CONFIG_SITE.Common.<target>
\end{verbatim}\index{CONFIG\_SITE.Common.\textless{}target\textgreater{}}\begin{description}\item Site specific target definitions for all hosts

\end{description}\begin{verbatim}CONFIG_SITE.<host>.Common
\end{verbatim}\index{CONFIG\_SITE.\textless{}host\textgreater{}.Common}\begin{description}\item Site specific host definitions for all targets
...
\begin{verbatim}convertRelease.pl
\end{verbatim}\index{convertRelease.pl}
\index{installEpics.pl}
\index{makeConfigAppInclude.pl}\begin{description}\item This Perl script does consistency checks for the external \textless{}top\textgreater{} definitions in the RELEASE file. This script also 
creates envPaths, cdCommands, and dllPath.bat files for vxWorks and other IOCs.

\end{description}\begin{verbatim}cvsclean.pl
\end{verbatim}\index{cvsclean.pl}\begin{description}\item This perl script finds and deletes cvs .\#* files in all directories of the directory tree.

\end{description}\begin{verbatim}dos2unix.pl
\end{verbatim}\index{dos2unix.pl}\begin{description}\item This perl script converts text file in DOS CR/LF format to unix ISO format.

\end{description}\begin{verbatim}expandVars.pl
\end{verbatim}\index{expanVars.pl}
\index{expandVars.pl}\begin{description}\item This perl tool expands @VAR@ variables while copying a file.

\end{description}\begin{verbatim}filterWarnings.pl
\end{verbatim}\index{filterWarnings.pl}\begin{description}\item This is a perl script that filters compiler warning output (for HP-UX).

\end{description}\begin{verbatim}fullpathname.pl
\end{verbatim}\index{fullpathname.pl}\begin{description}\item This perl script returns the fullpathname of a file.

\end{description}\begin{verbatim}installEpics.pl
\end{verbatim}\index{installEpics.pl}\begin{description}\item This is a Perl script that installs build created files into the install directories.

\end{description}\begin{verbatim}makeDbDepends.pl
\end{verbatim}\index{makeDbDepends.pl}\begin{description}\item This perl script searches .substitutions and .template files for entries to create a DEPENDS file.

\end{description}\begin{verbatim}makeIncludeDbd.pl
\end{verbatim}\index{makeIncludeDbd.pl}\begin{description}\item This perl script creates an include dbd file from file names

\end{description}\begin{verbatim}makeMakefile.pl
\end{verbatim}\index{makeMakefile.pl}\begin{description}\item This is a perl script that creates a Makefile in the created O.\textless{}arch\textgreater{} directories.

\end{description}\begin{verbatim}makeTestfile.pl
\end{verbatim}\index{makeTestfile.pl}\begin{description}\item This perl script generates a file \$target.t which executes a real test program in the same directory.

\end{description}\begin{verbatim}mkmf.pl
\end{verbatim}\index{mkmf.pl}
\index{mkmf.pl}\begin{description}\item This perl script generates include file dependencies for targets from source file include statements.

\end{description}\begin{verbatim}munch.pl
\end{verbatim}\index{munch.pl}\begin{description}\item This is a perl script that creates a ctdt.c file for vxWorks target arch builds which lists the c++ static constructors 
...
\end{verbatim}\index{mv.pl}
\index{replaceVAR.pl}\begin{description}\item This is a perl script that changes VAR(xxx) style macros in CapFast generated databases into the \$(xxx) notation 
used in EPICS databases.

\end{description}\begin{verbatim}useManifestTool.pl
\end{verbatim}\index{useManifestTool.pl}\begin{description}\item  This tools uses MS Visual C++ compiler version number to determine if we want to use the Manifest Tool 
...
\end{verbatim}\index{README.1st}\begin{description}\item Instructions for setup and building epics base

\end{description}\begin{verbatim}README.html
\end{verbatim}\index{README.html}\begin{description}\item html version of README.1st

\end{description}\begin{verbatim}README.MS_WINDOWS
\end{verbatim}\index{README.MS\_WINDOWS}\begin{description}\item Microsoft WIN32 specific instructions

\end{description}\begin{verbatim}README.niCpu030
\end{verbatim}\index{README.niCpu030}\begin{description}\item NI cpu030 specific instructions

\end{description}\begin{verbatim}README.hpux
\end{verbatim}\index{README.hpux}\begin{description}\item \index{hpux}HPUX 11 (hpux-parisc) specific instructions

\end{description}\begin{verbatim}README.cris
\end{verbatim}\index{README.cris}\begin{description}\item Cris architecture specific instructions

\end{description}\begin{verbatim}README.tru64unix
\end{verbatim}\index{README.tru64unix}
\index{README.tru64unix}\begin{description}\item Tru64Unix/Alpha specific instructions

\end{description}\begin{verbatim}README.darwin.html
\end{verbatim}\index{README.darwin.html}\begin{description}\item Installation notes for Mac OS X (Darwin)

\end{description}\begin{verbatim}BuildingR3.13AppsWithR3.14.html
\end{verbatim}\index{BuildingR3.13AppsWithR3.14.html}\begin{description}\item Describes how to modify a R3.13 vxWorks application so that it builds with release R3.14.1.

\end{description}\begin{verbatim}ConvertingR3.13AppsToR3.14.html
\end{verbatim}\index{ConvertingR3.13AppsToR3.14.html}\begin{description}\item Describes how to convert a R3.13 vxWorks application so that it contains a R3.14 configure directory and R3.14 
...
\end{verbatim}\index{BuildingR3.13ExtensionsWithR3.14.html}\begin{description}\item Describes how to modify a R3.13 extension so that it builds with release R3.14.1.

\end{description}\begin{verbatim}RELEASE_NOTES.html
\end{verbatim}\index{RELEASE\_NOTES.html}\begin{description}\item Describes changes in the R3.14.1 release

\end{description}\begin{verbatim}KnownProblems.html
\end{verbatim}\index{KnownProblems.html}\begin{description}\item List of known problems in EPICS base R3.14.1.

\end{description}\begin{verbatim}ReleaseChecklist.html
\end{verbatim}\index{ReleaseChecklist.html}\begin{description}\item Checklist of things that must be done when creating a new release of EPICS Base.

\end{description}\section{Startup Files}

\index{Startup files}
\index{Startup Files}\subsection{Base Startup Directory}
...
\end{verbatim}\index{EpicsHostArch}\begin{description}\item c shell script to set EPICS\_HOST\_ARCH environment variable

\end{description}\begin{verbatim}EpicsHostArch.pl
\end{verbatim}\index{EpicsHostArch.pl}\begin{description}\item perl script to set EPICS\_HOST\_ARCH environment variable

\end{description}\begin{verbatim}Site.profile
\end{verbatim}\index{Site.profile}\begin{description}\item Unix bourne shell script to set path and environment variables

\end{description}\begin{verbatim}Site.cshrc
\end{verbatim}\index{Site.cshrc}\begin{description}\item Unix c shell script to set path and environment variables

\end{description}\begin{verbatim}cygwin.bat
\end{verbatim}\index{cygwin.bat}\begin{description}\item WIN32 bat file to set path and environment variables for building with cygwin gcc/g++ compilers

\end{description}\begin{verbatim}win32.bat
\end{verbatim}\index{win32.bat}\begin{description}\item WIN32 bat file to set path and environment variables for building with MS Visual C++ compilers
