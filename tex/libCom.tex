...
standard \index{C++ library}C++ library where such facilities are not available or not efficient on all the target platforms on which EPICS is 
...
\index{epicsRingPointer.h}\verb|epicsRingPointer.h| describes a C++ and a C facility for a commonly used type of ring buffer.
...
\index{ringPointer}An epicsRingPointer cannot be assigned to, copy-constructed, or constructed without giving the \emph{size} argument. The C++ 
...
\index{epicsTimer}
\index{epicsTimer.h}\verb|epicsTimer.h| describes a C++ and a C timer facility.
...
\index{logClientId}    An abstract data type, representing a log client.
...
\index{logClientCreate}    Create a new log client. Will block the calling task for a maximum of 2 seconds trying to connect to a server with the 
...
\index{logClientSend}Send the given message to the given log client. Messages are not immediately sent to the log server. Instead they are sent 
...
\index{logClientFlush}    Immediately send all outstanding messages to the server.

\begin{verbatim}
void logClientShow (logClientId id, unsigned level);
\end{verbatim}

\index{logClientShow}    Print information about the log clients internal state to stdout.
...
\index{logClientInit}   Create a log client that uses the standard ioc log client environment variables (EPICS\_IOC\_LOG\_INET and 
...
\index{logClientSendMessage}    Check the global variable iocLogDisable before calling logClientSend.
...
\index{aToIPAddr}aToIPAddr() fills in the structure pointed to by the \emph{pIP} argument with the Internet address and port number specified by 
...
In all cases the `:p' may be omitted in which case the port number is set to the value of the \emph{defaultPort} argument. All 
...
\index{adjustToWorstCaseAlignment}adjustToWorstCaseAlignment() returns a value \textgreater{}= \emph{size} that an exact multiple of the worst case alignment for the 
...
\index{epicsConvertDoubleToFloat}\verb|epicsConvertDoubleToFloat| converts a double to a float. If the double value can not be represented as a float 
...
\index{epicsTypes.h}
\index{epicsTypes.h}\verb|epicsTypes.h| provides typedefs for architecture independent data types.
