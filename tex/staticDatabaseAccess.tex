...
\end{verbatim}NOTE: On an IOC \index{pdbbase}pdbbase is a global variable, which is accessable if you include dbAccess.h
...
\index{42500: UHEADING2: Database Configuration Field Types}Each database field has a type as defined in the next chapter. For static database access a new and simpler set of field 
...
\begin{itemize}\item \index{DCT\_STRING}DCT\_STRING: Character string.
...
\index{dbAllocBase}\begin{verbatim}DBBASE *dbAllocBase(void);
...
\index{dbFreeBase}\begin{verbatim}void dbFreeBase(DBBASE *pdbbase);
...
\index{Alloc/Free DBENTRY}\begin{verbatim}DBENTRY *dbAllocEntry(DBBASE *pdbbase);
void dbFreeEntry(DBENTRY *pdbentry);
\end{verbatim}\index{dbAllocEntry}
\index{dbFreeEntry}These routines allocate, initialize, and free \verb|DBENTRY| structures. The user can allocate and free \verb|DBENTRY| structures as 
...
\index{dbInitEntry}
\index{dbFinishEntry}\begin{verbatim}void dbInitEntry(DBBASE *pdbbase,DBENTRY *pdbentry);
void dbFinishEntry(DBENTRY *pdbentry);
\end{verbatim}\index{dbInitEntry}
\index{dbFinishEntry}The routines \verb|dbInitEntry| and \verb|dbFinishEntry| are provided in case the user wants to allocate a \verb|DBENTRY| structure 
...
\end{verbatim}\index{dbCopyEntry}
\index{dbCopyEntryContents}The routine d\verb|bCopyEntry| allocates a new entry, via a call to \verb|dbAllocEntry|, copies the information from the original 
...
\end{verbatim}\index{dbReadDatabase}
\index{dbReadDatabaseFP}
\index{dbPath}
\index{dbAddPath}\verb|dbReadDatabase| and \verb|dbReadDatabaseFP| both read a file containing database definitions as described in chapter 
...
\begin{verbatim}long dbWriteMenu(DBBASE *pdbbase,char *filename,char *menuName);
...
\end{verbatim}\index{dbWriteMenu}
\index{dbWriteMenuFP}
...
\index{dbWriteBreaktableFP}Each of these routines writes files in the same format accepted by \verb|dbReadDatabase| and \verb|dbReadDatabaseFP|. Two 
...
\begin{verbatim}long dbWriteRecord(DBBASE *pdbbase,char * file,

char *precordTypeName,int level);
long dbWriteRecordFP(DBBASE *pdbbase,FILE *fp,

char *precordTypeName,int level);
\end{verbatim}\index{dbWriteRecord}
\index{dbWriteRecordFP}Each of these routines writes files in the same format accepted by \verb|dbReadDatabase| and \verb|dbReadDatabaseFP|. Two 
versions of each type are provided. The only difference is that one accepts a filename and the other a "\verb|FILE| *". Thus only 
one of each type has to be described.
...
\end{verbatim}\index{dbGetNRecordTypes}This routine returns the number of record types in the database.
...
\end{verbatim}\index{dbFindRecordType}
\index{dbFirstRecordType}
\index{dbNextRecordType}\verb|dbFindRecordType| locates a particular record type. \verb|dbFirstRecordType| locates the first, in alphabetical order, 
...
\begin{verbatim}status = dbFirstRecordType(pdbentry);
while(!status) {
/*Do something*/
...
\end{verbatim}\index{dbGetRecordTypeName}This routine returns the name of the record type that DBENTRY currently references. This routine should only be called 
...
\end{verbatim}\index{dbGetNFields}Returns the number of fields for the record instance that DBENTRY currently references.
...
\end{verbatim}\index{dbFirstField}
\index{dbNextField}
\index{dbFoundField}These routines are used to locate fields. If any of these routines returns success, then DBENTRY references that field 
description. 

\subsection{Get Field Type}

\begin{verbatim}int  dbGetFieldType(DBENTRY *pdbentry);
\end{verbatim}\index{dbGetFieldType}This routine returns the integer value for a DCT field type, see Section14.2.3 on page198, for a description of the field 
types.

\subsection{Get Field Name}

\begin{verbatim}char *dbGetFieldName(DBENTRY *pdbentry);
\end{verbatim}\index{dbGetFieldName}This routine returns the name of the field that DBENTRY currently references. It returns NULL if DBENTRY does not 
point to a field.

\subsection{Get Default Value}

\begin{verbatim}char *dbGetDefault(DBENTRY *pdbentry);
\end{verbatim}\index{dbGetDefaultName}This routine returns the default value for the field that DBENTRY currently references. It returns NULL if DBENTRY 
...
\end{verbatim}\index{dbGetPrompt}
\index{dbGetPromptGroup}The \verb|dbGetPrompt| routine returns the character string prompt value, which describes the field. \verb|dbGetPromptGroup| 
...
\end{verbatim}\index{dbPutRecordAttribute}This creates or modifies the attribute \emph{name} on the record type referenced by \emph{pdbentry} to \emph{value}. Attribute names should be 
...
\end{verbatim}\index{dbGetRecordAttribute}Looks up the attribute \emph{name} for the record type referenced by \emph{pdbentry} and sets the the field pointer in \emph{pdbentry} to refer to 
...
\end{verbatim}\index{dbGetNRecords}Returns the total number of record instances and aliases for the record type that DBENTRY currently references.

\subsection{Get Number of Record Aliases}

\begin{verbatim}int dbGetNAliases(DBENTRY *pdbentry)
\end{verbatim}\index{dbGetNAliases}Returns the number of record aliases for the record type that DBENTRY currently references.
...
\end{verbatim}\index{dbFindRecord}
\index{dbFirstRecord}
\index{dbNextRecord}These routines are used to locate record instances and aliases. If any of these routines returns success, then DBENTRY 
...
\begin{verbatim}char *dbGetRecordName(DBENTRY *pdbentry);
\end{verbatim}\index{dbGetRecordName}This routine only works properly if called after \verb|dbFindRecord|, \verb|dbFirstRecord|, or \verb|dbNextRecord| has returned 
...
\end{verbatim}\index{dbIsAlias}This routine only works properly if called after \verb|dbFindRecord|, \verb|dbFirstRecord|, or \verb|dbNextRecord| has returned 
...
\end{verbatim}\index{dbCreateRecord}
\index{dbCreateAlias}
\index{dbDeleteRecord}
\index{dbDeleteAliases}
\index{dbFreeRecords}\verb|dbCreateRecord|, which assumes that \verb|DBENTRY| references a valid record type, creates a new record instance and 
...
\begin{verbatim}long dbCopyRecord(DBENTRY *pdbentry, char *newRecordName
int overWriteOK)
\end{verbatim}\index{dbCopyRecord}This routine copies the record instance currently referenced by \verb|DBENTRY| (it fails if DBENTRY references an alias). Thus 
...
\index{dbCopyRecord}\subsection{Rename Record}

\begin{verbatim}long dbRenameRecord(DBENTRY *pdbentry, char *newname)
\end{verbatim}\index{dbRenameRecord}This routine renames the record instance currently referenced by \verb|DBENTRY| (it fails if DBENTRY references an alias). If 
\verb|dbRenameRecord| completes successfully, DBENTRY references the renamed record.

\index{dbRenameRecord}\subsection{Record Visibility}
...
\end{verbatim}\index{dbVisibleRecord}
\index{dbInvisibleRecord}
\index{dbIsVisibleRecord}\verb|dbVisibleRecord| sets a record to be visible. \verb|dbInvisibleRecord| sets a record invisible. 
...
\end{verbatim}\index{dbFindField}
\index{dbFoundField}Given that a record instance has been located, \verb|dbFindField| finds the specified field. If it returns success, then 
...
\end{verbatim}\index{dbGetString}
\index{dbPutString}
\index{dbVerify}
\index{dbGetRange}
\index{dbIsDefaultValue}These routines are used to get or change field values. They work on all the database field types except \verb|DCT_NOACCESS| 
...
\end{verbatim}\index{dbGetNMenuChoices}This routine returns the number of menu choices for menu.

\subsection{Get Menu Choice}

\begin{verbatim}char **dbGetMenuChoices(DBENTRY *pdbentry);
\end{verbatim}\index{dbGetMenuChoices}This routine returns the address of an array of pointers to strings which contain the menu choices.
...
char *choice);
\end{verbatim}\index{dbGetMenuIndex}
\index{dbPutMenuIndex}
\index{dbGetMenuStringFromIndex}
\index{dbGetMenuIndexFromString}NOTE: These routines do not work if the current field value contains a macro definition.
...
\index{dbGetMenuStringFromIndex}
\index{dbGetMenuIndexFromString}\subsection{Locate Menu}

\begin{verbatim}dbMenu *dbFindMenu(DBBASE *pdbbase,char *name);
\end{verbatim}\index{dbFindMenu}\verb|dbFindMenu| is most useful for runtime use but is a static database access routine. This routine just finds a menu with the 
given name.
...
\begin{itemize}\item \index{DCT\_LINK\_CONSTANT}DCT\_LINK\_CONSTANT: Constant value.
...
\end{verbatim}\index{dbGetNLinks}
\index{dbGetLinkField}
\index{dbGetLinkType}These are routines for manipulating \verb|DCT_xxxLINK| fields. \verb|dbGetNLinks| and \verb|dbGetLinkField| are used to walk 
...
\end{verbatim}\index{dbCvtLinkToConstant}
\index{dbCvtLinkToPvlink}These routines should be used for modifying \verb|DCT_LINK_CONSTANT| or \verb|DCT_LINK_PV| links. They should not be used 
...
\end{verbatim}\index{dbAllocForm}
\index{dbFreeForm}\verb|dbAllocForm| allocates storage needed to manipulate forms. The return value is the number of elements in the form. If 
...
\end{verbatim}\index{dbGetFormPrompt}
\index{dbGetFormValue}
\index{dbPutForm}\verb|dbGetFormPrompt| returns a pointer to an array of pointers to character strings specifying the prompt string. 
...
\end{verbatim}\index{dbVerifyForm}\verb|dbVerifyForm| can be called to verify user input. It returns \verb|NULL| if no errors are present. If errors are present, it returns 
...
\end{verbatim}\index{dbFirstInfo}
\index{dbNextInfo}
\index{dbFindInfo}There are two ways to locate info items, by scanning through the list using first/next, or by asking for the item by name. 
...
\end{verbatim}\index{dbGetInfoName}Returns the name of the info item referred to by \verb|pdbentry|, or a NULL pointer if no item is referred to.
...
\end{verbatim}\index{dbGetInfoString}
\index{dbPutInfoString}These routines provide access to the currently selected items' string value. When changing the string value using 
...
\end{verbatim}\index{dbGetInfoPointer}
\index{dbPutInfoPointer}Each info item includes space to store a single \verb|void*| pointer as well as the value string. Applications using the info item 
...
\end{verbatim}\index{dbPutInfo}
\index{dbDeleteInfo}A new info item can be created by calling \verb|dbPutInfo|. If an item by that name already exists its value will be replaced 
...
\end{verbatim}\index{dbGetInfo}It is common to want to look up the value of a named info item in one call, and \verb|dbGetInfo| is provided for this purpose. 
It returns a NULL pointer if no info item exists with the given name.

\section{Find Breakpoint Table}

\begin{verbatim}brkTable *dbFindBrkTable(DBBASE *pdbbase,char *name)
\end{verbatim}\index{dbFindBrkTable}This routine returns the address of the specified breakpoint table. It is normally used by the runtime breakpoint conversion 
...
\index{dbPvdDump}
\index{dbReportDeviceConfig}These routines are used to dump information about the database. \verb|dbDumpRecord|, \verb|dbDumpMenu|, \verb|dbDumpDriver|,\verb| |
...
\index{dbExpand}This example is like the \verb|dbExpand| utility, except that it doesn't allow path or macro substitution options, It reads a set of 
...
\index{dbDumpRecords}NOTE: This example is similar but not identical to the actual \verb|dbDumpRecords| routine.
