...
\index{iocsh}
The first call to \verb|iocsh| executes commands from the startup script filename which must be passed as an argument to the 
...
The \index{envPaths}envPaths file is automatically generated in the IOC boot directory and defines several environment variables that are 
...
each support module referenced in the application's \verb|configure/RELEASE| file:
...
The \verb|cdCommands| script is automatically generated in the IOC boot directory and defines several vxWorks global 
...
application's \verb|configure/RELEASE| file:
...
The \verb|ld| command in the startup script loads EPICS core, the record, device and driver support the IOC needs, and any 
...
Initialize the logging system. See the chapter on ``IOC Error Logging'' for details.
