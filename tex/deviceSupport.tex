\chapter{\index{58383: LOCALTITLE: Chapter 8 Record And Device Support}Device Support}
...
\begin{itemize}\item \index{dtyp - dbCommon}dtyp:  Device Type.
...
\begin{verbatim}/* Create the dset for devAiSoft */
long init_record();
long read_ai();
struct {
long   number;
...
\end{verbatim}\index{synchronous device support example}The example is \verb|devAiSoft|, which supports soft analog inputs. The \verb|INP| field can be a constant or a database link or a 
...
\index{asynchronous device support example}This example shows how to write an asynchronous device support routine. It does the following sequence of operations:
...
\begin{verbatim}static void myCallback(CALLBACK *pcallback)
{
...
\begin{verbatim}long report(
int   interest);
\end{verbatim}\index{report - device support routine}This routine is responsible for reporting all I/O cards it has found. If \verb|interest| is (0,1) then generate a (short, long) 
...
\index{25600: HEADING2: 12.4.2 Initialize Record Processing}\begin{verbatim}long init(
int   after);
\end{verbatim}\index{init - device support routine}This routine is called twice at IOC initialization time. Any action is device specific. This routine is called twice: once 
...
\index{66790: HEADING2: 12.4.3 Initialize Specific Record}\begin{verbatim}long init_record(
void *precord);   /* addr of record*/
\end{verbatim}\index{init\_record - device support routine}The record support \verb|init_record| routine calls this routine.
...
struct dbCommon   *precord,
IOSCANPVT   *ppvt);
\end{verbatim}\index{get\_ioint\_info - device support routine}This is called by the I/O interrupt scan task. If \verb|cmd| is (0,1) then this routine is being called when the associated record is 
...
\index{65780: HEADING1: 12.5 Extended Device Support}This section describes the additional behaviour and routines required for a device support layer to support online changes 
...
\begin{enumerate}\item Records that were connected at iocInit to a device support layer that does not implement the extended interface are 
...
\end{verbatim}\index{struct dsxt}There may be future additions to this table to support additional functionality; such extensions may only be made by 
...
struct dbCommon *precord);
\end{verbatim}\index{add\_record}This function is called to offer a new record to the device support. It is also called during iocInit, in between the pass 0 and 
...
struct dbCommon *precord);
\end{verbatim}\index{del\_record}This function is called to notify the device support of a request to change the hardware address of a record, and allow the 
